...
test -n "$version" || { echo 'cannot determine quantus-miner version' >&2; exit 1; }
...
install -m 0755 "$binary" "$package/quantus-miner"
...
sha256sum "$binary" > "$output/quantus-miner-linux-x86_64.sha256.upstream"
