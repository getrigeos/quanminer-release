...
[[ -x $binary ]] || { echo "[h-run] quantus-miner binary is missing or not executable" >&2; exit 127; }
...
# quantus-miner accepts only IP:PORT for --node-addr: resolve a hostname at
...
if pgrep -x quantus-miner >/dev/null 2>&1; then
    echo "[h-run] quantus-miner is already running" >&2
    exit 1
fi

echo "[h-run] starting quantus-miner (pool credentials redacted)"
